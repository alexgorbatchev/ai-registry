...
- **Skill Template Variables:** `SKILL.md` files may use a small set of build-time `{{variable_name}}` placeholders for canonical local paths and other repo facts. Expansion happens during `bun run build` when skills are staged into generated outputs. Unknown placeholders must fail the build instead of being left unresolved.
