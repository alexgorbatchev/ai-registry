...
- **The Harnesses (`harnesses/`)**:
  ...
  - Current generated harnesses are OpenCode (`harnesses/opencode/`) and Pi (`harnesses/pi/`).
...
- **Unified Outputs:** The script generates final harness outputs in `.output/`. Today that includes `.output/opencode` for OpenCode and `.output/pi/<profile>` for Pi.
...
- **Public CLI Helpers:** Any wrapper script intended for `~/.local/bin/` symlinking (e.g., `pi-*`, `air-*`) MUST NOT be checked into `scripts/`. Instead, `scripts/build.ts` must generate them into `.output/bin/` during the build process, and `scripts/bootstrap.ts` will symlink them from there.
- **Bootstrap Command:** `bun run bootstrap` is the repo-local setup entrypoint. It installs dependencies, builds the generated outputs, links the default machine-local OpenCode target using an XDG-style path unless `OPENCODE_CONFIG_DIR` is set, and can optionally link one generated Pi profile with `-- --pi-profile <profile>` unless `PI_CODING_AGENT_DIR` is set.
...
- **Pi Harness Mapping:** Pi has no native equivalent to OpenCode's `tools` and `permission` manifest fields. The Pi harness consumes `description`, `skills`, `commands`, and `system_prompt`, and it fails the build if a profile uses non-empty `tools` or `permission` values. Each profile additionally merges harness-local assets found in `harnesses/pi/prompts/` and `harnesses/pi/skills/` into the final profile output. The build also generates `pi-<profile>` helper scripts in the output `bin/` directory, which are symlinked into `~/.local/bin/` by the bootstrap process to provide easy access to individual profile environments.
