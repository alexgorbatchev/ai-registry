...
- **Unified Outputs:** The script generates final harness outputs in `.output/`. Today that includes `.output/opencode` for OpenCode and `.output/pi/profiles/<profile>` for Pi.
...
- **Pi Harness Mapping:** Pi has no native equivalent to OpenCode's `tools` and `permission` manifest fields. The Pi harness consumes `description`, `skills`, `commands`, and `system_prompt`, and it fails the build if a profile uses non-empty `tools` or `permission` values.
