...
- **Bootstrap Command:** `bun run bootstrap` is the repo-local setup entrypoint. It installs dependencies, builds the generated outputs, links the default machine-local OpenCode target using an XDG-style path unless `OPENCODE_CONFIG_DIR` is set, links the generated Codex `default` profile using `${CODEX_HOME:-~/.codex}` unless `-- --codex-profile <profile>` overrides it, can optionally link one generated Pi profile with `-- --pi-profile <profile>` unless `PI_CODING_AGENT_DIR` is set, and always refreshes the generated public wrappers in `~/.local/bin`.
...
- **Pi Harness Mapping:** Pi has no native equivalent to OpenCode's `tools` and `permission` manifest fields. The Pi harness consumes `description`, `skills`, `commands`, and `system_prompt`, and it fails the build if a profile uses non-empty `tools` or `permission` values. Each profile additionally merges harness-local assets found in `harnesses/pi/prompts/` and `harnesses/pi/skills/` into the final profile output. The build also generates `pi` for the `default` profile plus `pi-<profile>` helper scripts for other generated profiles in the output `bin/` directory, which are symlinked into `~/.local/bin/` by the bootstrap process to provide easy access to individual profile environments.
