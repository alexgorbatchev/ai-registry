---
description: Comprehensive code review — full on first run, incremental on subsequent runs, writes {{ env "DOCS_INTERNAL_DIR" }}/audit/review.md
---

Perform a comprehensive code review of this codebase. The output is always a holistic review of the entire project written to `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` — but the exploration scope depends on whether a previous review exists. A past review file may already exist at that path; preserve and update its valid accumulated knowledge instead of ignoring it.

## Step 1: Determine review mode

Use `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` frontmatter as the baseline source of truth.

Expected `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` frontmatter:
...
' {{ env "DOCS_INTERNAL_DIR" }}/audit/review.md 2>/dev/null)
```

- **If `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` does not exist**: this is a **full review**. Proceed to Step 2a.
- **If `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` exists and `review_sha` is present and valid**: this is an **incremental review**. Use `review_sha` as your baseline and proceed to Step 2b.
- **If `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` exists but `review_sha` is missing/invalid**: treat this as a **full review** and regenerate `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` with valid frontmatter in Step 5.
...
Record these in the `# Project Review Runbook` section in `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` (Step 5).

Then skip to Step 3.

## Step 2b: Incremental review — explore only what changed

Read the existing `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` — this is your accumulated knowledge of the full codebase.
...
If `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` existed, check every issue listed against the current code:
...
Use commands from `# Project Review Runbook` in `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` first.
...
## Step 5: Write {{ env "DOCS_INTERNAL_DIR" }}/audit/review.md

Write `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md` as a complete, holistic review of the entire project — not just the changes. A past review file may already exist there; merge new findings with carried-forward issues from the previous review (if any). Remove issues that are fixed. The result should be a single coherent document that stands on its own.

At the top of `{{ env "DOCS_INTERNAL_DIR" }}/audit/review.md`, include YAML frontmatter with the current review metadata:
