...
- Create directories the harness tool owns at runtime (session stores, logs, todo lists) with `context.buildSupport.ensureRuntimeDirectory(...)`, not `mkdir`. That registers them as ensure-present, unmanaged output so the manifest never tracks them and the tool pruning them never shows up as drift. Codex uses it for `sessions/`, `log/`, and `.tmp/`; Pi for `sessions/`; Claude Code for the disposable ones only, keeping the rest as persistent stores outside `.output/` (see the Claude Code section).
...
- Materialize the disposable runtime directories Claude Code writes into (`file-history/`, `session-env/`, `statsig/`) in the default root through `buildSupport.ensureRuntimeDirectory(...)` so every profile shares one cache. They are ensure-present, unmanaged: excluded from `manifest.json`, never drift, never removed by sync, recreated when Claude Code prunes them. Treat everything Claude Code writes there as tool-owned.
- The persistent stores (`projects/` for per-project memory files and transcripts, `sessions/` for the history `claude --resume` reads, `todos/`, the `shell-snapshots/` those sessions restore from, and `plugins/` for installed plugins) hold work a rebuild must not destroy, so they must survive deleting `.output/`. The build keeps them under `${XDG_DATA_HOME:-~/.local/share}/ai-registry/claude-code/` and symlinks each into the generated `default` root, where the other profiles pick them up through the usual default-profile symlink. A build that still finds a real directory at one of those paths in the generated output moves it into the store once and fails loudly if both locations hold one. Add a directory to `PERSISTENT_STORE_DIR_NAMES` in the Claude Code build plugin, not `SHARED_RUNTIME_DIR_NAMES`, whenever losing the data in it would cost the user real work.
