...
- `opencode.db` - Main SQLite database for persisted OpenCode data
- `opencode.db-shm` and `opencode.db-wal` - SQLite companion files while the database is active
- `log/` - Application logs

Session history is stored in the SQLite database, primarily in the `session`, `message`, and `part` tables.

If you are troubleshooting a specific Git worktree, OpenCode may also create a repo-local `.git/opencode` file. That file is not the full session history. It stores the OpenCode `project.id` that links the repo back to rows in `~/.local/share/opencode/opencode.db`.

Some older installs may still have legacy JSON files under `~/.local/share/opencode/storage/`, but current OpenCode session storage is SQLite-backed.
