...
async function writeScriptFile(repositoryRoot: string, fileName: string, content: string = "#!/usr/bin/env bash\n"): Promise<string> {
  const scriptPath = join(repositoryRoot, fileName);
