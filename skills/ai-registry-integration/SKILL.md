...
- When checked-in guidance or generated text refers to repository paths, use the build-time template tags instead of machine-specific absolute paths. Supported string variables are `{{repo_root}}`, `{{skills_dir}}`, `{{commands_dir}}`, `{{profiles_dir}}`, `{{output_dir}}`, `{{file_path}}` (the original source file path being rendered), and `{{file_dir}}` (the original source file directory being rendered). The build also supports `&#123;&#123; include "path/from/repo/root.md" &#125;&#125;` and `&#123;&#123; env "VAR_NAME" &#125;&#125;` with optional `default` for generated text outputs.
...
- Public CLI helpers meant to be symlinked to `~/.local/bin/` (like `pi-*` or `air-*` wrappers) MUST NOT be checked into `{{repo_root}}/scripts/`. They must be generated by `{{repo_root}}/scripts/build.ts` into `{{repo_root}}/.output/bin/` so the bootstrap script can link them from there.
