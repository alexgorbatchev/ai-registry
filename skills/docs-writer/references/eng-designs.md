...
- Start the design doc with `created_on`, `last_modified`, and `status` YAML frontmatter so the doc follows the internal-doc lifecycle contract defined in the `docs-writer` skill.
...
- Refer to the baseline `docs-writer` skill for overall internal documentation lifecycle rules.
- **When implemented:** When an engineering design is fully implemented, follow the `Completed Engineering Designs Rule` in the baseline `docs-writer` skill to promote and move the completed document to `{{ env "DOCS_INTERNAL_DIR" }}/references/` as long-term, maintained reference documentation.
