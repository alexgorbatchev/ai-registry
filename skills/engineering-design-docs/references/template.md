...
Default new design docs to `{{ env "DOCS_INTERNAL_DIR" }}/eng-designs/<topic>.md`. Start the design doc with `created_on`, `last_modified`, and `status` YAML frontmatter. Archive superseded design docs by moving the document to `{{ env "DOCS_INTERNAL_DIR" }}/eng-designs/archived/<topic>.md`.
