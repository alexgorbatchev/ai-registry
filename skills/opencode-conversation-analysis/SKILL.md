...
OpenCode-specific. Supports SQLite-backed storage (`opencode.db`) with fallback to legacy JSON storage. Requires Bun.
...
```bash
bun ~/.agents/skills/opencode-conversation-analysis/scripts/extract.ts
```
...
- Outputs to `{{repo_root}}/.opencode-analysis/chunk_*.jsonl`
...
```
Read the file `{{repo_root}}/.opencode-analysis/chunk_N.jsonl` which contains user messages from coding sessions (JSONL format with fields: session_id, session_title, timestamp, text).
...
- **Different chunk sizes**: Edit `CHUNK_SIZE` in `extract.ts` (default 320000 chars)
- **Different message filter**: Edit `MIN_TEXT_LEN` in `extract.ts` (default 10 chars)
- **Include subagent sessions**: Remove the `parent_id IS NULL` (SQLite) and `parentID` (legacy) filters in `extract.ts`
- **Time period filtering**: Add timestamp filtering in `extract.ts`
