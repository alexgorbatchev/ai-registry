...
async function initSkill(
  ...
  const skillContent = formatTemplate(SKILL_TEMPLATE, {
  ...
  try {
    ...
    await Bun.write(
      exampleScriptPath,
      formatTemplate(EXAMPLE_SCRIPT, { skill_name: skillName }),
    );
    ...
    await Bun.write(
      ...
      formatTemplate(EXAMPLE_REFERENCE, { skill_title: skillTitle }),
    );
...
if (import.meta.main) {
  ...
  if (args.length < 3 || args[1] !== '--path' || !skillName || !pathArg) {
  ...
  const result = await initSkill(skillName, pathArg);
