...
//   execPath = /Users/x/.local/share/claude/versions/2.1.273
//   argv[1]  = /$bunfs/root/cli            (2.1.273; earlier builds reported
//                                            /$bunfs/root/src/entrypoints/cli.js)
// Only the "/$bunfs/" prefix is stable across builds, so that is all the gate
// tests. basename(execPath) is the version number, not "claude" — never check it.
...
if (isBunCompiledEntry && isClaudeBinary) {
  ...
  // A rejected gate is a silent no-op by design, so with debug on it must at
  // least say why — this is how argv1/execPath drift in a new claude build
  // gets diagnosed instead of "AGENTS.md just stopped loading".
  logger.LOG(
    `gate off: isBunCompiledEntry=${isBunCompiledEntry} isClaudeBinary=${isClaudeBinary} execPath=${process.execPath} argv1=${argv1}`
  );
}
...
function createLogger() {
  ...
  return { debug, LOG, LOG_PATH };
}

function patch({ debug, LOG, LOG_PATH }) {
  const fs = require("fs");
  ...
  // One line per content read, naming the branch it took. path-swap is the
  // line that proves a lone AGENTS.md was actually served; passthrough has
  // no twin on disk so it is omitted there.
  function logRead(kind, api, p, twin) {
    if (kind === "passthrough") {
      LOG(`read kind=${kind} api=${api} path=${p}`);
      return;
    }
    LOG(`read kind=${kind} api=${api} path=${p} twin=${twin}`);
  }
  ...
  function readResolved(p, options) {
    ...
    logRead(r.kind, "readFileSync", p, r.twin);
  ...
  fs.readFile = function (p, options, callback) {
    ...
    const opts = typeof options === "function" ? undefined : options;
    const r = resolve(p);
    logRead(r.kind, "readFile", p, r.twin);
  ...
  fs.promises.readFile = async function (p, options) {
    ...
    const encoding =
      typeof options === "string" ? options : options && options.encoding;
    logRead(r.kind, "promises.readFile", p, r.twin);
